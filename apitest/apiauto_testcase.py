...
def writeBug(bug_id,intterface_name,request,response,res_check):
    intterface_name = intterface_name.encode('utf-8')
    res_check = res_check.encode('utf-8')
    request = request.encode('utf-8')
    now = time.strftime("%Y-%m-%d %H:%M:%S")
    bugname = str(bug_id)+'_'+intterface_name.decode()+'_出错了'
    bugdetail = '[请求数据]<br />'+request.decode()+'<br />'+'[预期结果]<br />'+res_check.decode()+'<br/>'+'<br/>'+'[响应数据]<br />'+'<br/>'+response.decode()
    print(bugdetail)
    sql = "INSERT INTO 'bug_bug'('bugname','bugdetail','bugstatus','buglevel','bugcreater','bugassign','create_time','Product_id') VALUES('%s','%s','1','1','fx','fx','%s','2');"%(bugname,pymysql.escape_string(bugdetail),now)
    coon = pymysql.connect(user='root', passwd='123456',db='autotest', port=3306,host='127.0.0.1',charset='utf8')
    cursor = coon.cursor()
    cursor.execute(sql)
    coon.commit()
